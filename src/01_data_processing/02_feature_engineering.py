...
依 ABJ (Anderson, Banker & Janakiraman 2003) 與 study.md 的設計：
  ...
  控制變數：SIZE、AI、EI、ROA、LEV、SUCC_DEC
  對連續變數做前後 1% 縮尾（_w 欄）。
...
WINSOR_COLS = [
    ...
    "WASTE_FINE_INT", "WATER_INTENSITY",
    "SIZE", "AI", "EI", "ROA", "LEV",
]
...
def main():
    ...
    df["DEC"] = np.where(both, (rev < rev_lag).astype(float), np.nan)

    # 連續兩年營收下降 SUCC_DEC（D_t=1 且 D_{t-1}=1）
    d_lag = lag("DEC")
    df["SUCC_DEC"] = np.where(
        df["DEC"].notna() & d_lag.notna(),
        ((df["DEC"] == 1) & (d_lag == 1)).astype(float),
        np.nan,
    )
    ...
    df["WASTE_DISC_GRI"] = (gri.fillna(0) > 0).astype(float)  # 做法 B（穩健性）
    ...
    df["WASTE_FINE_INT"] = fine_amt / (assets_ * 1000.0) * 1e6
    ...
    LAG_TARGETS = ["WATER_RATE_w", "製程水回收率%_w", "WATER_DISC", "WASTE_DISC_GRI",
                   "WASTE_INTENSITY_w", "WASTE_DISC", "WASTE_FINE_w",
                   "WASTE_FINE_INT_w", "WATER_INTENSITY_w"]
    ...
    key = ["dLNSGA", "dLNREV", "DEC", "SUCC_DEC", "WATER_RATE", "WATER_DISC",
    ...
    # 迴歸最低要求（三者皆有）
    core_mask = df[["dLNSGA", "dLNREV", "DEC"]].notna().all(axis=1)
