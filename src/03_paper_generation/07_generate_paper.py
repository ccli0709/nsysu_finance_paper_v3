...
DESC_VARS = ["Y_dLNSGA", "dLNREV", "DEC", "WATER_RATE", "WATER_DISC",
             "WASTE_INTENSITY", "WASTE_DISC", "WASTE_FINE",
             "SIZE", "AI", "EI", "ROA", "LEV", "SUCC_DEC"]
...
def build_models_table(coef, specs):
    c0 = coef[coef["遞延期"] == 0]
    role_rows = [("β1", "ΔLNREV", "dLNREV"), ("β2", "DEC×ΔLNREV", "DEC_x_dREV"),
                 ("β3", "環境變數×D×ΔLNREV", None), ("β4", "環境變數", None),
                 ("", "SIZE×DEC×ΔLNREV", "SIZE_D_dREV"), ("", "AI×DEC×ΔLNREV", "AI_D_dREV"),
                 ("", "EI×DEC×ΔLNREV", "EI_D_dREV"), ("", "ROA×DEC×ΔLNREV", "ROA_D_dREV"),
                 ("", "LEV×DEC×ΔLNREV", "LEV_D_dREV"), ("", "SUCC_DEC×DEC×ΔLNREV", "SUCC_DEC_D_dREV")]
...
def main():
    ...
    add_para(doc,
             "環境管理測試變數（Env_Var）包括六項變數：核心實質使用密集度變數「用水密集度」（WATER_INTENSITY，單位：立方公尺/百萬元營收）與「廢棄物密集度」（WASTE_INTENSITY，單位：公噸/百萬元營收）；輔助揭露與違規變數「水回收率」（WATER_RATE，%）、「水揭露」（WATER_DISC，虛擬變數）、「廢棄物揭露」（WASTE_DISC，遵循 GRI 標準分級）與「環境裁罰金額」（WASTE_FINE，裁罰金額/總資產）。控制變數包含公司規模（SIZE）、資產密集度（AI）、員工密集度（EI）、ROA、負債比率（LEV）以及連續兩期營收下降虛擬變數（SUCC_DEC）。")
