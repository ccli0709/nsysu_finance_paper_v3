...
para(doc, "應變數 ΔLNSGA（營業費用變動）；核心為 ΔLNREV、收入下降虛擬 D，與環境變數 "
          "Env_Var 之三重交乘。實證模型如下：")
para(doc, "ΔLNSGA = β0 + β1·ΔLNREV + β2·(D×ΔLNREV) + β3·(Env_Var×D×ΔLNREV) "
          "+ β4·Env_Var + Σ Controls×(D×ΔLNREV) + Σ Industry + Σ Year + ε", indent=False)
para(doc, "β2 捕捉整體成本黏性（預期為負）；β3 為核心係數。Env_Var 依模型分別代入六項"
          "環境管理變數：模型1 水回收率%(H1)、模型2 水揭露(H2)、模型3 每百萬營收廢棄物"
          "(H3)、模型4 GRI廢棄物揭露度(H4)、模型5 事業廢棄物罰鍰次數(H5)。", indent=False)
para(doc, "控制變數：企業規模、資產密集度、員工密集度、ROA、財務槓桿、連續兩年營收下降；"
          "並控制產業(SASB)與年份固定效果，採公司叢集穩健標準誤、連續變數縮尾 1%。另檢驗"
          "當期與遞延（t-1、t-2），並依 TSE 產業別分高／低耗水（高污染）兩組檢驗異質性。"
          "資料取自 TEJ，台灣製造業、排除金融、2014–2024，面板 18,137 觀測（1,945 家）。")
